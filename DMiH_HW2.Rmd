...
```{r}
t.test(x = subset(heart_df$adiposity, heart_df$famhist==1),
		y = subset(heart_df$adiposity, heart_df$famhist==0),
...
```{r}
var.test(x = subset(heart_df$adiposity, heart_df$famhist==1),
			y = subset(heart_df$adiposity, heart_df$famhist==0),         
			alternative = "two.sided", 
			conf.level = 0.95)
```
...
### **(1)** 
#### **Add a new column “BWeightG” to group babies’ weights into three groups: $Weight \le 2500 g$, $2500 g < Weight \le 4000 g$, and $Weight > 4000 g$.**

```{r}
# summary(birth_filter)
new_birth <- birth_filter %>% mutate(BWeightG = cut(Weight, breaks= c(-Inf, 2500, 4000, Inf), labels=c("low", "medium", "high")))
# summary(new_birth)
