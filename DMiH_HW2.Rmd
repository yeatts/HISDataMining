...
p-value: **8.561e-05**\
95 percent confidence interval: **1.444960 4.286862**\
...
p-value: **0.2799**\
95 percent confidence interval: **0.6660296 1.1269203**\
...
library(ggpubr)
data(birth)
birth_filter = filter(birth, Country == "FR")
```
...
y = change  
consuming wine w/o weight-based exercies  
consuming wine w weight-based exercies  
not consuming wine w/o weight-based exercies  
not consuming wine with weight-based exercies  
...
The assumptions of our model are not reasonable based on the information provided below.
```{r}
par(mfrow = c(2,2))
plot(diet_fit)

# test - X and residuals are uncorrelated
cor.test(diet$change, diet_fit$residuals)
# test - the mean of residulas is zero
mean(diet_fit$residuals)
# test - the variance of residuals are equal for all predicted values
# false
# test - resdiuals follow normal distribution
# no
