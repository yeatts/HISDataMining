...
library(catdata)
library(plyr) 
# plyr loaded before dplyr becuase of console warning... 
# using plyr:: and dplyr:: notation for clarity dealing with masked objects
library(dplyr)
data(birth)
str(birth)
```
...
```{r}
#table(birth$Country)
plyr::count(birth, 'Country') #using count() from plyr package
```
***
...
```{r}
country_freq = plyr::count(birth, 'Country')
country_freq
...
```{r}
country_freq = dplyr::arrange(country_freq, desc(freq))
country_freq
# fun code example for Jeremy ****can be deleted**** 
filter(country_freq, freq == max(freq)) 
```
...
amTable = table(birth_fr$AgeMother) 
names(amTable)[amTable == max(amTable)] # another way to get mode(s)
...
```{r}
plot(year_birth_fr$Year, year_birth_fr$avg_mom_age,  type = "o", 
     xlab = "Year", ylab = "Age", main = "Average moms' age by year in France", 
     col = "blue", lwd = 2, ylim = c(20, 28))
```
...
***
### **(5)** 
#### **Add a new column “preterm_rate” in “preterm_brith_fr” to show the preterm rate for each year from 1998 to 2004. Delete the column “count” in “preterm_birth_fr”.**
